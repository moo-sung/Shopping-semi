...
List<PostDTO> postList = PostDAO.getDAO().selectAllPostListGeneral("QUESTION", startRow, endRow, search, keyword);

//공지사항 NOTICE의 전체 게시글의 갯수를 계산하여 저장
int totalPost = PostDAO.getDAO().selectPostCount("QUESTION", search, keyword);
...
<body>
	<div class="main">
		...
		<table>
			...
			<tr id="body">
				...
				<td class="item"><a href="#"><%=ProductDAO.getDAO().selectProduct(post.getProduct_idx()).getName()%></a></td>
				<td class="subject"><a href="<%=request.getContextPath()%>/index.jsp?workgroup=wontack&work=question_detail&pageNum=<%=pageNum%>&postIdx=<%=post.getIdx()%>">
				<%if(ReplyDAO.getDAO().selectReply(post.getIdx())!=null) { %> <span style="border: 0.5px solid; padding: 3px; font-size: 7px;">답변완료</span>&nbsp;&nbsp; <% } %>
				<%=post.getTitle()%></a></td>
				<td class="writer">
				<%
			...
			<tr>
				<td colspan="5">
					<form action="<%=request.getContextPath()%>/index.jsp?workgroup=wontack&work=question&pageNum=1" method="post" id="search">
	...
	$("#writeBtn").click(function() {
		...
		<% } else {%>
		location.href= "<%=request.getContextPath()%>/index.jsp?workgroup=wontack&work=question_write";
		<%}%>
	});
	</script>	
</body>
