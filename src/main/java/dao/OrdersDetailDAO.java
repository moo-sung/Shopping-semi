...
public class OrdersDetailDAO extends JdbcDAO {
	...
	public int insertOrdersDetail(OrdersDetailDTO ordersDetail) {
		...
		try {
			...
			String sql = "insert into OrdersDetail values(? || TO_CHAR(orders_detail_seq.nextval),?,?,?,?)";
			...
			System.out.println("OrdersDetail 테이블 insert 오류 => " + e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	public int updateOrdersDetail(OrdersDetailDTO ordersDetail) {
		...
		try {
			...
			System.out.println("OrdersDetail 테이블 update 오류 => " + e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	public int deleteOrdersDetail(String idx) {
		...
		try {
			...
			String sql = "delete from OrdersDetail where idx = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, idx);

			rows = pstmt.executeUpdate();
		} catch (Exception e) {
			System.out.println("OrdersDetail 테이블 delete 오류 => " + e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	public OrdersDetailDTO selectOrdersDetail(String idx) {
		...
		try {
			...
			String sql = "select * from OrdersDetail where idx = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, idx);
	...
	// select 
	public OrdersDetailDTO selectOrderDetailIDXList(String orders_idx) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		OrdersDetailDTO ordersDetail = null;
		try {
			...
			String sql = "select * from OrdersDetail where orders_idx = ?";
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("OrdersDetail 테이블 selectOrdersIDX 오류 => " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return ordersDetail;
	...
	public List<OrdersDetailDTO> selectAllOrdersDetailList(String orderBy) {
		...
		try {
			...
			String sql = "select * from OrdersDetail order by ?";
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("OrdersDetail 테이블 selectAll 오류 => " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return list;
